...
```
프로젝트명에 공백을 넣으니 쉘 스크립트 만들때 공백 때문에 문제가 된다. 
프로젝트명을 변경했다.

좌측 메뉴공간 맨 아래 rename 이 있고, 해당 메뉴로 변경한다.

Dashboard
jenkins_springbatch_0001
Rename
```
...
### 배포 쉘 스크립트 파일 
```
colaman@vm-dev-00:~/WORKS/APPS/jenkins_springbatch_0001$ cat ./deploy_jenkins_springbatch_0001.sh 
#!/bin/bash
...
echo "END TASK"
echo
colaman@vm-dev-00:~/WORKS/APPS/jenkins_springbatch_0001$ 
...
```
colaman@vm-dev-00:~/WORKS/APPS/jenkins_springbatch_0001$ cat ./run.sh 
#!/bin/bash
...
### 실행결과
```
colaman@vm-dev-00:~/WORKS/APPS/jenkins_springbatch_0001$ ./run.sh 20240219
[APP_PATH] /home/colaman/WORKS/APPS/jenkins_springbatch_0001
[APP_FILE_NAME] jenkins_springbatch_0001-0.0.1-SNAPSHOT.jar
[PARAM_REQUEST_DATE] 20240219
...
...
2024-02-18 16:13:10.883  INFO 4921 --- [ionShutdownHook] com.zaxxer.hikari.HikariDataSource       : HikariPool-1 - Shutdown completed.
colaman@vm-dev-00:~/WORKS/APPS/jenkins_springbatch_0001$ 
```
...
젠킨스는 jenkins 계정으로 실행된다.

```
colaman@vm-dev-00:~/WORKS/APPS/jenkins_springbatch_0001$ ps -ef | grep -v grep | grep jenkins
jenkins      706       1  4 15:32 ?        00:01:53 /usr/bin/java -Djava.awt.headless=true -jar /usr/share/java/jenkins.war --webroot=/var/cache/jenkins/war --httpPort=18080
colaman@vm-dev-00:~/WORKS/APPS/jenkins_springbatch_0001$ 
```

```
추가 스크립트도 젠킨스 계정으로 실행되는데,
배포 위치는 일반 유저 홈 이라, 권한 문제로 퍼미션 디나이드(permission denied) 에러가 난다.

배포 위치를 jenkins 계정과, 운영계정(colaman) 과 같이 할 수 있는 곳으로 해야 한다.

루트 디렉토리부터 공용 디렉토리로 갈 수 있으나,
일단, devops 라는 그룹을 만들고 colaman, jenkins 를 그룹원으로 넣는다.

이후 디렉토리별 권한을 적당히 조정한다.

su 명령어로 jenkins 계정으로 변경하고(colaman -> root -> jenkins) 배포스크립트를 먼저 테스트 한다.
동작에 이상이 없으면, 젠킨스 등록을 한다.

```

### 4.2 젠킨스 등록
```
Build steps 에 Execute Shell 로 추가 등록을 한다.
아래 텍스트 에이리어에 쉘 정보를 넣는다. (풀패스 경로명으로 파일명까지 넣는다)
```
...
```
...
[jenkins_springbatch_0001] $ /bin/sh -xe /tmp/jenkins9885655061497367144.sh
+ /home/colaman/WORKS/APPS/jenkins_springbatch_0001/deploy_jenkins_springbatch_0001.sh
[SRC_PATH] /var/lib/jenkins/workspace/jenkins_springbatch_0001/target/jenkins_springbatch_0001-0.0.1-SNAPSHOT.jar
[DEST_PATH] /home/colaman/WORKS/APPS/jenkins_springbatch_0001
[DATE_TIMESTAMP] 20240218_160407
[APP_FILE_NAME_BACKUP] jenkins_springbatch_0001-0.0.1-SNAPSHOT.jar.BACKUP.20240218_160407
